  ...
  <div className="flex flex-col grow items-center py-4">
    <div className='grid grid-cols-3 items-center w-2/3'>
      ...
      <div className='text-right'>
        <FontAwesomeIcon icon={faPlus} size="lg" className='text-lunar-green-200 cursor-pointer' onClick={() => HandleClickAddCategory()} />
      </div>
    </div>
            ...
            <table className='text-lunar-green-300 border-collapse w-full'>
              <tbody>
                <tr className='border-b border-lunar-green-200 h-12'>
                  ...
                  <td className='text-right'>
                    <input type='number' value={newImportance} onChange={(e) => setNewImportance(e.target.value)} placeholder='Set an importance...' className='rounded-sm w-full bg-transparent text-lunar-green-100' />
                  </td>
                </tr>
              </tbody>
            </table>
            <div className='overflow-y-auto w-full' style={{ maxHeight: 'calc(100vh - 400px)' }}>
              <table className='text-lunar-green-300 border-collapse w-full'>
                <tbody className='text-lunar-green-100'>
                  {categories.map((category, index) => (
                    <tr key={index} className='border-b border-lunar-green-200 h-12' onClick={() => handleEditCategory(category)}>
                      ...
                      <td className='border-r border-lunar-green-200 w-3/5 text-left'>{category.name}</td>
